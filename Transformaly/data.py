...
class PascalVOCDataModule():
    ...
    def __init__(self, batch_size, normal_classes, image_train_transform,  data_dir="./VOCSegmentation", num_workers=2) -> None:
        super().__init__()
        ...
        self.val_dataset = VOCDataset(self.dir, image_set="val", transform=self.image_train_transform, normal_classes = self.normal_classes)
        self.test_dataset = VOCDataset(self.dir, image_set="val", transform=self.image_train_transform, normal_classes = self.normal_classes)

        self.test_dataset_normal = []
        self.test_dataset_ood = []
        for i in range(len(self.test_dataset)):
            if self.test_dataset[i][1] == 0:
                self.test_dataset_normal.append(self.test_dataset[i])
            else:
                self.test_dataset_ood.append(self.test_dataset[i])

        #self.test_dataset_normal = torch.utils.data.DataLoader(self.test_dataset_normal, batch_size=self.batch_size, shuffle=False)
        #self.test_dataset_ood = torch.utils.data.DataLoader(self.test_dataset_ood, batch_size=self.batch_size, shuffle=False)

        print(f"Train set size : {len(self.train_dataset)}")
        print(f"Test set size : {len(self.test_dataset)}")
        print(f"Test set normal size : {len(self.test_dataset_normal)}")
        print(f"Test set ood size : {len(self.test_dataset_ood)}")
    ...
    def get_normal_test_dataloader(self, batch_size=None):
        batch_size = self.batch_size if batch_size is None else batch_size
        return DataLoader(self.test_dataset_normal, batch_size=batch_size, shuffle=False, num_workers=self.num_workers, pin_memory=True)

    def get_ood_test_dataloader(self, batch_size=None):
        batch_size = self.batch_size if batch_size is None else batch_size
        return DataLoader(self.test_dataset_ood, batch_size=batch_size, shuffle=False, num_workers=self.num_workers, pin_memory=True)
...
class Coco_Handler(Dataset):
    def __init__(self, batch_size, normal_classes, transformations, num_workers):
        ...
        # Make a subset of the test_dataset with all the instances that have label 0 and another subset with all the instances that have label 1
        ...
        self.test_loader_normal = torch.utils.data.DataLoader(self.test_dataset_normal, batch_size=self.batch_size, shuffle=False, num_workers=self.num_workers)
        self.test_loader_ood = torch.utils.data.DataLoader(self.test_dataset_ood, batch_size=self.batch_size, shuffle=False, num_workers=self.num_workers)
    ...
    def get_test_loader_normal(self):
        return self.test_loader_normal
    
    def get_test_loader_ood(self):
        return self.test_loader_ood
    ...
    def get_test_dataset_normal(self):
        return self.test_dataset_normal
    
    def get_test_dataset_ood(self):
        return self.test_dataset_ood
